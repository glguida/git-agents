...
Human operators may use `job-reset`, `job-kill`, or agent reset porcelain to
recover from stuck or interrupted agents. Treat an unexpected status/owner change
as an instruction to stop working on that job and let the launcher exit.
